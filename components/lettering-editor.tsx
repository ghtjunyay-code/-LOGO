...
export default function LetteringEditor({spot:s,update,background='#ffffff'}:{background?:string;spot:Spot;update:(fn:(s:Spot)=>void)=>void}){
 ...
 {t.role==='個人名'?<div id={`${s.id}-lettering-${t.role}`} tabIndex={-1}><PersonalNamesEditor perLocation value={t.personal||newPersonalNames(t.text)} hasCompany={v.items.some(a=>a.role==='会社名')} change={fn=>itemChange(t.role,x=>{x.personal=x.personal||newPersonalNames(x.text);fn(x.personal);if(!v.items.some(a=>a.role==='会社名')){x.personal.place='同じ場所';x.personal.noName=false}x.text=previewName(x.personal);if(!allowedLetteringFonts(personalText(x.personal)).includes(canonicalFont(x.font)))x.font='楷書体'})}/></div>:<> <label className="field" htmlFor={`${s.id}-lettering-${t.role}`}>{t.role}の刺繍文字 <em>必須</em><input id={`${s.id}-lettering-${t.role}`} value={t.text} maxLength={80} aria-required="true" placeholder={t.role==='会社名'?'例：株式会社 山口建設':'例：山田 太郎'} onChange={e=>itemChange(t.role,x=>{x.text=e.target.value;if(!allowedLetteringFonts(x.text).includes(canonicalFont(x.font)))x.font='楷書体'})}/></label></>}

 {t.role!=='個人名'&&appearance}
 {(hasLetteringGap(t,v)&&!(v.noLogo&&v.items[0]===t))&&<div className="grid2">{(t.role!=='個人名'||!v.items.some(c=>c.role==='会社名'))&&<Choice label="ロゴに対する配置" value={t.position} options={letteringPositions} onChange={a=>itemChange(t.role,x=>{x.position=a})}/>}<Choice label="間隔" value={`${t.gap} mm`} options={Array.from({length:201},(_,i)=>`${i} mm`)} onChange={a=>itemChange(t.role,x=>{x.gap=parseInt(a)})}/></div>}
 </div>})}
 <GuideEditor value={v.guide||{kind:'ポケット',slope:'右上がり'} as const} onChange={g=>change(x=>{x.guide=g;if(g.kind==='基準なし'){x.logoDirection='地面に水平';x.items.forEach(t=>{if(!t.personal||t.personal.place!=='異なる場所')t.direction='地面に水平'})}})} label="この刺繍箇所の基準"/>
 {[...(v.noLogo?[]:['ロゴ']),...v.items.filter(t=>!t.personal?.noName).map(t=>t.role)].map(target=>{const item=v.items.find(t=>t.role===target);const separate=!v.perLocation&&target==='個人名'&&item?.personal&&(item.personal.place==='異なる場所'||!v.items.some(t=>t.role==='会社名'));const guide=separate?(item.guide||{kind:'基準なし',slope:'分からない'} as const):(v.guide||{kind:'ポケット',slope:'右上がり'} as const);return <div key={target}>
