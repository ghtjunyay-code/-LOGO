...
export default function LetteringPreview({spot:s,background}:{spot:Spot;background:string}){
 ...
 <PreviewDimensions width={l.w} height={l.h} rotation={l.logoRotation} color={dimensionColor}/>
 ...
 <p className="muted">ロゴ：横 {s.width||'未指定'} × 縦 {s.height||'未指定'} mm。{guideSummary(guide)}。{guide.kind==='ポケット'&&'実線はポケット上端の参考線です。ポケットの幅は11センチぐらいを想定。'}会社名の横幅は書体から算出した参考値です。形・傾き・距離と書体は参考表示です。実際の商品に合わせて仕上げます。</p>
