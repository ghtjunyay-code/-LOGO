...
export function LogoPreview({spot,background='#ffffff'}:{spot:Spot;background?:string}){
 ...
 <rect x={x-12} y={y-12} width={w+24} height={h+24} fill={background} stroke="#bfcbd6"/><image href={assetUrl(displayAsset(spot.asset))} x={x} y={y} width={w} height={h} preserveAspectRatio="none"/>
 ...
 </svg></div><p className="muted">ボディ色を背景にした寸法の参考図です。周囲の背景・余白を除いたロゴ本体の寸法です。</p></>;
}
