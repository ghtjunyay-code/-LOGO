...
export default function ThreadPicker({spot:s,update}:{spot:Spot;update:(fn:(s:Spot)=>void)=>void}){
 ...
 useEffect(()=>{if(s.threadMode!=='指定する'||!asset||hasRows)return;let active=true;setStatus('ロゴの色を自動判別しています…');
 ...
 return()=>{active=false};
 },[asset?.id,s.threadMode,hasRows,retry]);
 ...
 {asset&&<button className="quiet" onClick={()=>{update(x=>{delete x.threadSelections});setRetry(n=>n+1)}}>ロゴから選び直す</button>}
