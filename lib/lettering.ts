...
export function parseLettering(raw:unknown):Lettering{
 ...
 if(v.direction!==undefined&&!['ポケット・縫い目の角度に合わせる','地面に水平'].includes(v.direction))return fail();return {enabled:v.enabled,pocketAngle:v.pocketAngle,items,...(v.direction?{direction:v.direction}:{})};
}
export function letteringSummary(v?:Lettering){if(!v?.enabled)return '';return `文字の向き：${v.direction||'地面に水平'}\n`+v.items.map(t=>`${t.role}：${t.text}\n${t.font}／文字高さ ${t.size} mm／${threadColors.find(c=>c.number===t.thread)?.name}（糸番号 ${t.thread}）\n配置：ロゴの${t.position}／間隔 ${t.gap} mm`).join('\n');}
