...
export function personalSummary(p:PersonalNames,perLocation=false){return (perLocation?'':`個人名を入れる場所：${p.place==='同じ場所'?`会社名と同じ場所／会社名の${p.position}／${p.align}${p.align==='中央寄せ'&&['上','下'].includes(p.position)?`（${p.centerBasis||'会社名の中央'}）`:''}`:`会社名と異なる場所／${p.location==='その他'?p.other:p.location}`}\n`)+p.rows.map(r=>`${r.size}・${r.quantity}着：${p.noName?'個人名なし（会社名のみ）':rowName(r)}`).join('\n')+`\n合計数量：${p.rows.reduce((n,r)=>n+r.quantity,0)}着`;}
