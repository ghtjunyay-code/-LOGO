...
export type PersonalNames={place:'同じ場所'|'異なる場所';location:string;other:string;position:string;align:string;noName:boolean;previewId:string;rows:NameRow[]};
...
export function personalSummary(p:PersonalNames){return `個人名を入れる場所：${p.place==='同じ場所'?`会社名と同じ場所／会社名の${p.position}／${p.align}`:`会社名と異なる場所／${p.location==='その他'?p.other:p.location}`}\n`+p.rows.map(r=>`${r.size}・${r.quantity}着：${p.noName?'個人名なし（会社名のみ）':rowName(r)}`).join('\n')+`\n合計数量：${p.rows.reduce((n,r)=>n+r.quantity,0)}着`;}
export function parsePersonalNames(raw:unknown):PersonalNames{
 ...
 return {place:p.place,location:p.location,other:p.other,position:p.position,align:p.align,noName:p.noName,previewId:ids.has(p.previewId)?p.previewId:rows[0].id,rows};
}
