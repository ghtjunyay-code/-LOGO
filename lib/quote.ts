...
export type Quote={version:1;id:string;customerId:string;process:'logo-embroidery';sourceId:string|null;customer:{name:string;company:string;email:string;phone:string};products:Product[];delivery:string;deliveryNotes:string;checked:boolean;step:number;updatedAt:string};
...
export const newQuote=():Quote=>({version:1,id:uid(),customerId:uid(),process:'logo-embroidery',sourceId:null,customer:{name:'',company:'',email:'',phone:''},products:[newProduct()],delivery:'',deliveryNotes:'',checked:false,step:1,updatedAt:new Date().toISOString()});
...
export function validate(q:Quote):Issue[]{const out:Issue[]=[];const add=(step:number,target:string,message:string)=>out.push({step,target,message});
 if(!q.customer.name.trim())add(1,'customer-name','お名前を入力してください');
 if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(q.customer.email))add(1,'customer-email','メールアドレスを確認してください');
