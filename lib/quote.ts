...
export type Spot={id:string;location:string;other:string;width:string;height:string;lock:boolean;axis?:'width'|'height';threadSelections?:{source:string;number:string}[];threadMode:string;thread:string;pattern:string;reference:string;notes:string;asset:Asset|null};
...
export const newSpot=():Spot=>({id:uid(),location:'左胸ポケット上',other:'',width:'',height:'',lock:true,threadMode:'業者に相談',thread:'',pattern:'不明',reference:'',notes:'',asset:null});
