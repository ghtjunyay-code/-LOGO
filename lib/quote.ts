...
export function manualThreads(q:Quote):Quote{const next=structuredClone(q);for(const p of next.products)for(const s of p.spots){if(s.threadSelections!==undefined){s.threadSelections=s.threadSelections.filter(r=>r.colorName!==undefined||r.confirmed!==false).map(r=>({...r,colorName:r.colorName??threadColors.find(c=>c.number===r.number)?.name??''}));s.thread=manualThreadText(s.threadSelections)}}return next}
