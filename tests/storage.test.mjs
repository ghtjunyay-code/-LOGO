...
import {manualThreads,newQuote,newSize,uid} from '../lib/quote.ts';
...
test('storage lifecycle, conflict, dispatcher header protection, immutable history and search',async()=>{const q=newQuote();q.customer.name='TEST storage';q.customer.company='テスト組織';q.customer.phone='000-0000-0000';const p=q.products[0];Object.assign(p,{name:'TEST garment',color:'紺',material:'不明',manageNo:'TEST-MANAGE'});p.sizes=[{...newSize(),size:'M',quantity:'4'}];const logo=new Uint8Array(Buffer.from('iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+aXioAAAAASUVORK5CYII=','base64'));let r=await fetch(base+'/api/files',{method:'POST',headers:{'Origin':base,'oai-authenticated-user-id':who,'Content-Type':'image/png','x-file-name':'test.png'},body:logo});assert.equal(r.status,200,await r.clone().text());const asset=await r.json();Object.assign(p.spots[0],{width:'30',height:'30',asset:{...asset,width:1,height:1}});p.previewColor="#112244";p.spots[0].axis="width";p.spots[0].location="背面えり下";p.spots[0].threadSelections=[{source:"#ffffff",number:"1198",confirmed:true}];p.spots[0].threadMode="指定する";p.spots[0].thread="白（糸番号 1198）";p.spots[0].asset.preview={id:asset.id,width:1,height:1};p.spots[0].lettering={enabled:true,pocketAngle:-8,items:[{...newLetteringItem('会社名'),text:'会社名テスト'},{...newLetteringItem('個人名'),text:'個人名テスト',font:'ゴシック',thread:'9022',position:'上',gap:10,angle:15}]};q.checked=true;
 ...
 const missingPreview=structuredClone(q);missingPreview.id=uid();missingPreview.products[0].spots[0].asset.preview.id=uid();r=await request("/api/quotes","POST",{data:missingPreview,status:"draft",revision:0});assert.equal(r.status,400);q.step=3;r=await request('/api/quotes','POST',{data:q,status:'draft',revision:1});assert.equal(r.status,200);assert.equal((await r.json()).revision,2);
 r=await request('/api/quotes','POST',{data:q,status:'draft',revision:1});assert.equal(r.status,409);
 r=await request('/api/quotes?id='+q.id);const restored=await r.json();assert.equal(restored.data.step,3);assert.deepEqual(restored.data.products,manualThreads(q).products);
